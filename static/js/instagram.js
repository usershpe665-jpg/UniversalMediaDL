/**
 * LunarMediaDL — Instagram Downloader Logic
...
(function App() {
  ...
  fetchBtn.addEventListener('click', async () => {
    ...
    try {
      ...
      $('videoTitle').textContent   = meta.title    || 'Instagram Media';
      $('videoChannel').textContent = meta.uploader || 'Instagram User';
      $('videoThumb').src           = meta.thumbnail || '';
      if (meta.duration_string) $('videoDuration').textContent = meta.duration_string;
      UI.showStep('stepInfo');
    } catch (err) {
      UI.toast(err.message || 'Error fetching Instagram info', 'error');
    } finally {
      fetchBtn.disabled = false;
  ...
  function onComplete(status) {
    ...
    History.add({
      jobId: status.job_id, title: meta?.title || 'Instagram Media',
      uploader: meta?.uploader || 'Instagram', thumbnail: meta?.thumbnail || '',
  ...
  // ─── History ───────────────────────────────────────────────────────────────
  const History = _buildHistoryModule('lunarytdl_instagram_history');

  function _buildHistoryModule(KEY) {
    const ht=$('historyToggleBtn'),hp=$('historyPanel'),hb=$('historyBackdrop'),
          hc=$('historyCloseBtn'), hl=$('historyList'), he=$('historyEmpty'), ch=$('clearHistoryBtn');

    const load  = () => { try { return JSON.parse(localStorage.getItem(KEY)||'[]'); } catch { return []; } };
    const save  = i  => { try { localStorage.setItem(KEY,JSON.stringify(i.slice(0,100))); } catch {} };
    const add   = e  => { const i=load().filter(x=>x.jobId!==e.jobId); i.unshift(e); save(i); };
    const del   = id => save(load().filter(i=>i.jobId!==id));
    const clear = () => { try { localStorage.removeItem(KEY); } catch {} };
    const esc   = s  => String(s).replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
    const rt    = ts => { const d=Date.now()-ts; return d<60e3?'Just now':d<3600e3?`${Math.floor(d/60e3)}m ago`:d<86400e3?`${Math.floor(d/3600e3)}h ago`:`${Math.floor(d/86400e3)}d ago`; };
  ...
  document.addEventListener('DOMContentLoaded', () => {
    UI.init();
    History.render();
    console.info('%cLunarMediaDL Instagram Module (MeTube engine)', 'color:#e879f9;font-size:14px;font-weight:bold');
  });
})();
