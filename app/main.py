...
class Config:
    ...
    # ── YouTube extractor args: mobile/iOS clients bypass JS signature solving ─
    # yt-dlp tries these clients IN ORDER; ios/mweb work without Node.js, but
    # having Node.js in the container makes 'web' work too (higher quality).
    _YT_EXTRACTOR_ARGS = {
        "youtube": {
            # Priority: ios (no JS needed) → mweb → web_creator → web (needs JS)
            "player_client":  ["ios", "mweb", "web_creator", "web"],
            # Skip DASH/HLS for ios client — direct stream is faster
            # "skip": ["dash"],   # uncomment if you only want progressive streams
        }
    }
    ...
    def _load_ytdl_options(self):
        ...
        # ── Inject default extractor_args unless user overrode them ─────────
        # This makes downloads work even when Node.js is absent (Railway cold
        # containers, CI, etc.) by preferring mobile clients that skip JS challenges.
        if "extractor_args" not in self.YTDL_OPTIONS:
            self.YTDL_OPTIONS["extractor_args"] = self._YT_EXTRACTOR_ARGS
        else:
            # Merge: preserve user's keys but ensure youtube.player_client exists
            yt_args = self.YTDL_OPTIONS["extractor_args"].setdefault("youtube", {})
            if "player_client" not in yt_args:
                yt_args["player_client"] = self._YT_EXTRACTOR_ARGS["youtube"]["player_client"]
...
async def metadata(request):
    ...
    def _extract():
        opts = dict(config.YTDL_OPTIONS)   # includes cookiefile + extractor_args

        is_playlist = bool(body.get("playlist", False))
        opts.update({
            "quiet":                    True,
            "no_color":                 True,
        ...
        if is_playlist:
            # For playlists: flat extraction is fine (only need titles/thumbnails)
            opts["extract_flat"] = "in_playlist"
        else:
            # For single videos: do NOT use extract_flat=True — it skips format
            # resolution and returns no formats, causing "Only images available".
            # We use process_ie_result=False instead, which fetches full info
            # without running postprocessors.
...
def _check_nodejs() -> str:
    """
    Detect Node.js availability for yt-dlp YouTube signature solving.
    yt-dlp uses Node.js to solve the 'n' challenge — without it, YouTube
    downloads silently fall back to iOS/mweb player_client (still works,
    but may miss some DASH streams). With Node.js, 'web' client is also
    available giving the highest quality streams.
    """
    ...
    try:
        ...
        if r.returncode == 0:
            ...
            return f"available ({ver}) — YouTube signature solving enabled"
        return "not found — falling back to ios/mweb player_client"
...
if __name__ == "__main__":
    ...
    log.info(f"Node.js      : {nodejs_status}")
    log.info(f"player_client: {config.YTDL_OPTIONS.get('extractor_args',{}).get('youtube',{}).get('player_client','[default]')}")
