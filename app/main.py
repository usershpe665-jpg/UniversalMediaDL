...
try:
    ...
    if 'ui' in str(e):
        raise RuntimeError('Could not find the frontend UI static assets. Make sure the ui/ directory exists next to app/') from e
    raise e
...
if __name__ == '__main__':
    ...
    # Load cookies from COOKIES environment variable (plain Netscape format, no base64 needed)
    cookies_env = os.environ.get('COOKIES', '').strip()
    if cookies_env:
        log.info('Loading cookies from COOKIES environment variable')
        os.makedirs(os.path.dirname(COOKIES_PATH) if os.path.dirname(COOKIES_PATH) else '.', exist_ok=True)
        with open(COOKIES_PATH, 'w', encoding='utf-8') as _cf:
            _cf.write(cookies_env)
        config.set_runtime_override('cookiefile', COOKIES_PATH)
        log.info(f'Cookie file written from COOKIES env var ({len(cookies_env)} chars)')
    elif os.path.exists(COOKIES_PATH):
        # Auto-detect cookie file on startup (fallback)
        config.set_runtime_override('cookiefile', COOKIES_PATH)
